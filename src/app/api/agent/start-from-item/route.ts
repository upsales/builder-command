...
export async function POST(request: NextRequest) {
  const { prompt, source, source_id, agent_prompt } = await request.json();
  if (!prompt) {
  ...
  db.prepare(
    "INSERT INTO daily_todos (id, date, text, sort_order, agent_enabled, source, source_id, agent_prompt) VALUES (?, ?, ?, ?, 1, ?, ?, ?)"
  ).run(todoId, today, prompt.slice(0, 500), maxOrder + 1, source ?? null, source_id ?? null, agent_prompt ?? null);
